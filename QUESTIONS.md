...
48. cannot instantiate array of type parameter. why ?
    can create a generic ArrayList though

    ```java
    public class NumberList1<E extends Number> implements List<E> {
        private E[] arrStore;
        private Integer[] istore;
        private ArrayList<E> listStore;
    
        public NumberList1() {
            this.istore = new Integer[100]; // works
            this.arrStore = new E[100]; // does not work
            this.listStore = new ArrayList<>();
        }
    }
    ```
    
49. so should we always use wildcards (in generics) ?
